...
const BotonCalificacion = () => {
    ...
    const enviarCalificacion = () => {
        ...
        axios.post('poner link aqui', data)
            .then(response => {
    ...
    const footerContent = (
        <div>
            <Button label="Cancelar" icon="pi pi-times" onClick={cancelarModal} className="p-button-text" />
            <Button label="Enviar" icon="pi pi-check" onClick={enviarCalificacion} autoFocus />
        </div>
    );

    return (
        <div className="card flex justify-content-center">
            <Button className="botonesPerfilProfesional"
                    label="Dejar Calificación"
                    icon="pi pi-star"
                    rounded 
                    onClick={() => setVisible(true)} />
            <Dialog header="¡Deja tu calificación!" visible={visible} footer={footerContent} style={{ width: '50vw' }} onHide={() => setVisible(false)}>
                <div className="contentModal">
                    ...
                    <div className="resena">
                        ...
                        <div>
                            <InputTextarea value={resena} onChange={(e) => setResena(e.target.value)} />
                        </div>
                    </div>
                </div>
            </Dialog>
